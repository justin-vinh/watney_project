...
.patient-list-row:hover{{background:#f0f4ff;cursor:pointer;}}
    ...
    with lock_overlay:
        ...
        with step1:
            ...
            def launch_demo():
                ...
                # Resolve demo CSV relative to this source file
                demo_path = Path(__file__).parent / 'watney_demo_data.csv'
                if not demo_path.exists():
                    ui.notify(f'Demo file not found: {demo_path}', color='red')
                    return
                try:
                    df = load_dataframe(str(demo_path))
                except Exception as e:
                    ui.notify(f'Could not load demo data: {e}', color='red')
                    return
                CURRENT_USER = user_select.value.strip() or 'Demo User'
                EXTRACTION_CSV_PATH = str(demo_path)
                # Don't save to config — demo mode is session-only
                _finish_login(demo=True)

            ui.button('Try Demo', on_click=launch_demo).props('outline').classes('w-64 text-gray-500')
            ui.label('3 synthetic brain cancer patients · no data saved').classes('text-xs text-gray-400')
    ...
    # ── Render patient ────────────────────────────────────────────────────────
    _demo_mode = [False]  # mutable flag so nested closures can read it
    ...
    def render_patient(index, demo=None):
        ...
        with left_panel:
            ...
            if _demo_mode[0]:
                ui.label('⚠ DEMO MODE — synthetic data').classes('text-xs text-amber-600 font-bold')
            ui.separator().classes('mb-4')
            ...
            def render_events(active_agent=None):
                ...
                # Check if any event matches the active agent
                ...
                if any_match:
                    highlight_notice.set_text(
                        f'▶ Highlighted card: likely progression event for {active_agent}'
                    )
                else:
                    highlight_notice.set_text('')
